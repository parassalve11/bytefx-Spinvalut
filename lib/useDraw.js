...
import { offsetForIndex, indexForOffset, spinPlan } from "./reel";
import { spinMotion, spinDuration, DEFAULT_FINAL_STEPS } from "./spinAnimation";
...
/** How long the landed card stays in view, highlighted, before the winner card opens. */
...
export function useDraw({ onSignedOut } = {}) {
  ...
  /** First-load progress for the loading screen: giveaway -> pending -> clients. */
  const [loadStep, setLoadStep] = useState("giveaway");
  ...
  const spinTo = useCallback(
    (recorded, config) => {
      ...
      const { anchor, target, order } = spinPlan(activeIndexRef.current, recorded.winnerIndex, len, config.minTravel);

      // Large pools: the winner trades places with a card that is off screen,
      // so travel stays readable. The cards on screen right now are unchanged.
      if (order) setLockedPool(order.map((i) => participants[i]));
      ...
      const duration = spinDuration(config.duration, finalSteps);
      const motion = reduced ? null : spinMotion(target - anchor, duration, finalSteps);
      ...
      animationRef.current = animate(x, [from, to], {
        ...
        ease: reduced ? "easeOut" : motion.ease,
        onUpdate: (value) => {
          if (reduced) return;
          const progress = (value - from) / (to - from);
          const next = progress < motion.launchEnd ? "launching" : progress < motion.slowFrom ? "spinning" : "slowing";
          if (next !== phase) {
            phase = next;
            setStatus(next);
          }
