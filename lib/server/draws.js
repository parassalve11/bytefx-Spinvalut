...
export async function listGiveaways(ibId) {
  return await db().prepare('SELECT id, name, created FROM giveaways WHERE ib_id = ? ORDER BY created DESC').all(ibId);
}
export async function ensureGiveaway(ibId) {
  return transaction(async connection => {
    const existing = await connection.prepare('SELECT id, name, created FROM giveaways WHERE ib_id = ? ORDER BY created DESC LIMIT 1').get(ibId);
    if (existing) return existing;
    const giveaway = { id: randomUUID(), name: 'My giveaway', created: Date.now() };
    await connection.prepare('INSERT INTO giveaways VALUES (?, ?, ?, ?)').run(giveaway.id, ibId, giveaway.name, giveaway.created);
    return giveaway;
  });
}
async function ownedGiveaway(ibId, giveawayId, connection = db()) {
  const giveaway = await connection.prepare('SELECT * FROM giveaways WHERE id = ? AND ib_id = ?').get(giveawayId, ibId);
  if (!giveaway) throw new AppError('Giveaway not found.', 404);
  return giveaway;
}
export async function createGiveaway(ibId, name) {
  if (typeof name !== 'string' || !name.trim() || name.trim().length > 80) throw new AppError('Enter a giveaway name of 1 to 80 characters.');
  return transaction(async connection => {
    if (await connection.prepare('SELECT id FROM draws WHERE ib_id = ? AND completed = 0').get(ibId)) throw new AppError('Reveal your pending draw before starting another giveaway.', 409, 'DRAW_PENDING');
...
export async function createPool(session, body) {
  await ownedGiveaway(session.profile.id, body.giveawayId);
  ...
  if (body.sourceSnapshotId) {
    ...
    network = await snapshotFor(session.profile.id, body.sourceSnapshotId);
  ...
  await db().prepare('DELETE FROM snapshots WHERE created < ?').run(Date.now() - 24 * 60 * 60 * 1000);
  await db().prepare('INSERT INTO snapshots VALUES (?, ?, ?, ?, ?)').run(snapshot.id, session.profile.id, body.giveawayId, encrypt(JSON.stringify(snapshot)), snapshot.created);
  return getPool(session.profile.id, snapshot.id, body.hideSensitive === true);
}
async function snapshotFor(ibId, id, connection = db()) {
  const row = await connection.prepare('SELECT * FROM snapshots WHERE id = ? AND ib_id = ?').get(id, ibId);
  if (!row) throw new AppError('Please refresh your client list.', 409, 'POOL_EXPIRED');
  return readPayload(row);
}
export async function getPool(ibId, id, hide = false) {
  const snapshot = await snapshotFor(ibId, id);
...
export async function historyFor(ibId, giveawayId) {
  await ownedGiveaway(ibId, giveawayId);
  return (await db().prepare('SELECT id, payload, created FROM draws WHERE ib_id = ? AND giveaway_id = ? ORDER BY created DESC LIMIT 30').all(ibId, giveawayId)).map(row => {
    const draw = readPayload(row);
    return { id: row.id, winnerName: draw.winner.name, at: draw.receipt.at, poolSize: draw.receipt.poolSize };
  });
}
export async function pendingDraw(ibId, hide = false) {
  const row = await db().prepare('SELECT payload FROM draws WHERE ib_id = ? AND completed = 0').get(ibId);
  return row ? presentDraw(readPayload(row), hide) : null;
}
export async function selectWinner(ibId, body) {
  ...
  return transaction(async connection => {
    const previous = await connection.prepare('SELECT * FROM draws WHERE ib_id = ? AND request_id = ?').get(ibId, body.requestId);
    if (previous) {
    ...
    const pending = await connection.prepare('SELECT id FROM draws WHERE ib_id = ? AND completed = 0').get(ibId);
    if (pending) throw new AppError('A draw is already waiting to be revealed. Recover it before drawing again.', 409, 'DRAW_PENDING');
    await ownedGiveaway(ibId, body.giveawayId, connection);
    const snapshot = await snapshotFor(ibId, body.snapshotId, connection);
    ...
    await connection.prepare('INSERT INTO draws (id, ib_id, giveaway_id, request_id, fingerprint, payload, created) VALUES (?, ?, ?, ?, ?, ?, ?)').run(id, ibId, body.giveawayId, body.requestId, fingerprint, encrypt(JSON.stringify(draw)), Date.now());
    return presentDraw(draw, criteria.hideSensitive);
  });
}
export async function completeDraw(ibId, id) {
  const row = await db().prepare('SELECT id FROM draws WHERE id = ? AND ib_id = ?').get(id, ibId);
  if (!row) throw new AppError('Draw not found.', 404);
  await db().prepare('UPDATE draws SET completed = 1 WHERE id = ? AND ib_id = ?').run(id, ibId);
}
