...
export default function EligibilityPanel({ criteria, onChange, onHideSensitiveChange, eligibleCount, totalCount, capabilities, levels, counts, onLevelsChange, onRefresh, poolStatus, poolError, disabled }) {
  const excluded = totalCount - eligibleCount;
  return (
    <div className="p-5 sm:p-6">
      <header className="mb-5 flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.08] pb-4">
        ...
        {/* One live region for the count and for background updates, so neither needs reserved space. */}
        <p className="text-xs text-text-muted" role="status" aria-live="polite">
          {poolStatus === "loading" ? (
            <span className="inline-flex items-center gap-2"><span className="pool-update-spinner" aria-hidden="true" />Updating clients…</span>
          ) : (
            <><span className="font-semibold text-accent-from">{eligibleCount}</span> eligible{excluded > 0 ? ` · ${excluded} filtered out` : ""}</>
          )}
        </p>
      </header>
      {poolStatus === "error" && poolError ? (
        <div role="alert" className="pool-error mb-5">
          <span>{poolError}</span>
          <button type="button" onClick={onRefresh} className="font-semibold text-accent-from underline underline-offset-4">Try again</button>
        </div>
      ) : null}
      <LevelSelector levels={levels} counts={counts} total={totalCount} onChange={onLevelsChange} disabled={disabled} />
      <fieldset disabled={disabled} aria-labelledby="eligibility-heading" className="eligibility-fields mt-5 grid min-w-0 gap-6 border-t border-white/[0.08] pt-5 disabled:opacity-55 md:grid-cols-[1fr_1.2fr]">
